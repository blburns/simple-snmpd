...
SNMPConfig::SNMPConfig()
    : port_(161), community_("public"), max_connections_(100),
      timeout_seconds_(30), log_level_("info"), enable_ipv6_(true),
      enable_trap_(false), trap_port_(162) {}
...
bool SNMPConfig::parse_config_value(const std::string &key,
  ...
  if (key == "port") {
    ...
    try {
      ...
      Logger::get_instance().log(LogLevel::ERROR,
                                 "Invalid max_connections value: " + value);
      return false;
    }
    ...
    try {
      ...
      if (timeout_seconds_ < 1) {
        Logger::get_instance().log(LogLevel::ERROR,
                                   "Invalid timeout_seconds: " + value);
        return false;
      }
    } catch (const std::exception &) {
      Logger::get_instance().log(LogLevel::ERROR,
                                 "Invalid timeout_seconds value: " + value);
      return false;
    }
    ...
    try {
      ...
      if (trap_port_ < 1 || trap_port_ > 65535) {
        Logger::get_instance().log(LogLevel::ERROR,
                                   "Invalid trap_port: " + value);
        return false;
      }
    } catch (const std::exception &) {
      Logger::get_instance().log(LogLevel::ERROR,
                                 "Invalid trap_port value: " + value);
      return false;
    }
  } else {
...
uint16_t SNMPConfig::get_port() const { return port_; }

const std::string &SNMPConfig::get_community() const { return community_; }

uint32_t SNMPConfig::get_max_connections() const { return max_connections_; }

uint32_t SNMPConfig::get_timeout_seconds() const { return timeout_seconds_; }

const std::string &SNMPConfig::get_log_level() const { return log_level_; }

bool SNMPConfig::is_ipv6_enabled() const { return enable_ipv6_; }

bool SNMPConfig::is_trap_enabled() const { return enable_trap_; }

uint16_t SNMPConfig::get_trap_port() const { return trap_port_; }

void SNMPConfig::set_port(uint16_t port) { port_ = port; }
...
void SNMPConfig::set_ipv6_enabled(bool enabled) { enable_ipv6_ = enabled; }

void SNMPConfig::set_trap_enabled(bool enabled) { enable_trap_ = enabled; }

void SNMPConfig::set_trap_port(uint16_t port) { trap_port_ = port; }

...
SNMPPacket::SNMPPacket()
    : version_(SNMP_VERSION_2C)
    , pdu_type_(SNMP_PDU_GET_REQUEST)
    , request_id_(0)
    , error_status_(0)
    , error_index_(0) {
}
...
bool SNMPPacket::parse(const uint8_t* data, size_t length) {
    ...
    if (offset >= length || data[offset] != 0x30) { // SEQUENCE
        Logger::get_instance().log(LogLevel::ERROR, "Invalid SNMP packet: missing sequence header");
        return false;
    }
    offset++;
    ...
    if (!parse_length(data, length, offset, packet_length)) {
        Logger::get_instance().log(LogLevel::ERROR, "Invalid SNMP packet: invalid length");
        return false;
    }

    if (packet_length != length - offset) {
        Logger::get_instance().log(LogLevel::ERROR, "Invalid SNMP packet: length mismatch");
        return false;
    }

    // Parse version
    if (offset >= length || data[offset] != 0x02) { // INTEGER
        Logger::get_instance().log(LogLevel::ERROR, "Invalid SNMP packet: missing version");
        return false;
    }
    offset++;
    ...
    if (version_length == 1) {
        ...
        Logger::get_instance().log(LogLevel::ERROR, "Invalid SNMP packet: invalid version length");
        return false;
    }
    ...
    if (offset >= length || data[offset] != 0x04) { // OCTET STRING
        Logger::get_instance().log(LogLevel::ERROR, "Invalid SNMP packet: missing community");
        return false;
    }
    offset++;
    ...
    community_.assign(reinterpret_cast<const char*>(data + offset), community_length);
    offset += community_length;

    // Parse PDU
    if (offset >= length) {
        Logger::get_instance().log(LogLevel::ERROR, "Invalid SNMP packet: missing PDU");
        return false;
    }
...
bool SNMPPacket::parse_length(const uint8_t* data, size_t length, size_t& offset, size_t& len) {
    if (offset >= length) {
        return false;
    }
...
bool SNMPPacket::parse_pdu_fields(const uint8_t* data, size_t& offset, size_t length) {
...
bool SNMPPacket::parse_variable_bindings(const uint8_t* data, size_t& offset, size_t length) {
    size_t start_offset = offset;
...
bool SNMPPacket::parse_variable_binding(const uint8_t* data, size_t& offset, size_t length, VariableBinding& varbind) {
    size_t start_offset = offset;

    // OID
    if (offset >= start_offset + length || data[offset] != 0x06) { // OBJECT IDENTIFIER
        return false;
    }
    offset++;
...
bool SNMPPacket::serialize_variable_bindings(std::vector<uint8_t>& buffer) const {
...
// Getters and setters
uint8_t SNMPPacket::get_version() const {
    return version_;
}

uint8_t SNMPPacket::get_pdu_type() const {
    return pdu_type_;
}

const std::string& SNMPPacket::get_community() const {
    return community_;
}

uint32_t SNMPPacket::get_request_id() const {
    return request_id_;
}

uint8_t SNMPPacket::get_error_status() const {
    return error_status_;
}

uint8_t SNMPPacket::get_error_index() const {
    return error_index_;
}

const std::vector<SNMPPacket::VariableBinding>& SNMPPacket::get_variable_bindings() const {
    return variable_bindings_;
}

void SNMPPacket::set_version(uint8_t version) {
    version_ = version;
}

void SNMPPacket::set_pdu_type(uint8_t pdu_type) {
    pdu_type_ = pdu_type;
}
...
void SNMPPacket::clear_variable_bindings() {
    variable_bindings_.clear();
}

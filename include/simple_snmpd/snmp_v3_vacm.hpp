...
public:
  ...
  bool remove_access(const std::string &group_name,
                     const std::string &context_prefix);
  bool update_access(const VACMAccess &access);
  VACMAccess *get_access(const std::string &group_name,
                         const std::string &context_prefix);
  ...
  // Access control
  bool is_read_allowed(const std::string &username, const std::string &context,
                       const std::string &oid,
                       VACMSecurityLevel security_level) const;

  bool is_write_allowed(const std::string &username, const std::string &context,
                        const std::string &oid,
                        VACMSecurityLevel security_level) const;

  bool is_notify_allowed(const std::string &username,
                         const std::string &context, const std::string &oid,
                         VACMSecurityLevel security_level) const;

  // View checking
  bool is_oid_in_view(const std::string &view_name,
                      const std::string &oid) const;

  // Configuration
  void set_default_read_view(const std::string &view_name) {
    default_read_view_ = view_name;
  }
  void set_default_write_view(const std::string &view_name) {
    default_write_view_ = view_name;
  }
  void set_default_notify_view(const std::string &view_name) {
    default_notify_view_ = view_name;
  }
  ...
  struct Statistics {
    ...
    Statistics()
        : total_checks(0), read_allowed(0), read_denied(0), write_allowed(0),
          write_denied(0), notify_allowed(0), notify_denied(0), view_checks(0),
          group_lookups(0), access_lookups(0) {}
  };
...
private:
  ...
  bool oid_matches_subtree(const std::string &oid,
                           const std::string &subtree) const;

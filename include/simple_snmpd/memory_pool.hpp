...
template <size_t BlockSize, size_t PoolSize> class MemoryPool {
public:
  MemoryPool();
  ~MemoryPool();
  ...
  struct Statistics {
    ...
    Statistics()
        : total_blocks(PoolSize), allocated_blocks(0), free_blocks(PoolSize),
...
public:
  ...
  struct Statistics {
    ...
    Statistics()
        : total_allocated(0), current_usage(0), peak_usage(0),
...
public:
  ...
  struct Statistics {
    ...
    MemoryPool<64, 1000>::Statistics
        variable_binding_stats; // 64 bytes per varbind
...
// Utility functions
std::string
memory_pool_statistics_to_string(const SNMPMemoryPool::Statistics &stats);
std::string variable_memory_pool_statistics_to_string(
    const VariableMemoryPool::Statistics &stats);

...
public:
    ...
    // Key generation
    bool generate_auth_key(const std::string& username, const std::string& password, 
                          SNMPv3AuthProtocol protocol, std::vector<uint8_t>& key);
    bool generate_priv_key(const std::string& username, const std::string& password, 
                          SNMPv3PrivProtocol protocol, std::vector<uint8_t>& key);
    ...
    struct Statistics {
        ...
        Statistics() : total_requests(0), auth_successes(0), auth_failures(0),

...
struct MIBEntry {
    ...
    MIBEntry(const std::vector<uint8_t>& o, const std::string& n, SNMPDataType t, bool ro = true)
...
struct MIBTableEntry {
    ...
    MIBTableEntry(const std::vector<uint8_t>& o, const std::string& n, SNMPDataType t, bool ro = true)
        : oid(o), name(n), type(t), read_only(ro) {}
};
...
public:
    ...
    bool get_next_oid(const std::vector<uint8_t>& oid, std::vector<uint8_t>& next_oid) const;
...
private:
    ...
    // Helper functions
    bool oid_matches(const std::vector<uint8_t>& oid, const std::vector<uint8_t>& pattern) const;
    bool oid_less_than(const std::vector<uint8_t>& oid1, const std::vector<uint8_t>& oid2) const;
...
public:
    ...
    static bool is_prefix(const std::vector<uint8_t>& oid1, const std::vector<uint8_t>& oid2);
    
    // Get the next OID in lexicographic order
    static std::vector<uint8_t> get_next_oid(const std::vector<uint8_t>& oid);
    
    // Compare two OIDs
    static int compare_oids(const std::vector<uint8_t>& oid1, const std::vector<uint8_t>& oid2);
};

...
struct HealthCheckResult {
    ...
    HealthCheckResult(HealthStatus s, const std::string& msg) 
        : status(s), message(msg), timestamp(std::chrono::system_clock::now()), duration(0) {}
};
...
class HealthCheck {
public:
    HealthCheck(const std::string& name, const std::string& description, 
               HealthCheckFunction check_func, std::chrono::milliseconds interval = std::chrono::milliseconds(5000));
    ~HealthCheck() = default;
...
public:
    ...
    // Configuration
    void set_monitoring_interval(std::chrono::milliseconds interval) { monitoring_interval_ = interval; }
    std::chrono::milliseconds get_monitoring_interval() const { return monitoring_interval_; }
    ...
    struct Statistics {
        ...
        Statistics() : total_checks_run(0), successful_checks(0), failed_checks(0),
...
private:
    ...
    // Update statistics
    void update_statistics(const HealthCheckResult& result, std::chrono::milliseconds duration);
};
...
std::string generate_health_json(const std::map<std::string, HealthCheckResult>& results);

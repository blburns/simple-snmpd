...
public:
  ...
  struct Statistics {
    ...
    Statistics()
        : total_tasks_processed(0), total_tasks_enqueued(0), max_queue_size(0),
          current_queue_size(0), max_active_threads(0),
          current_active_threads(0), total_processing_time(0),
...
public:
  ...
  std::future<SNMPRequestHandler::Response>
  process_request_async(const SNMPRequestHandler::Request &request);

  // Process request synchronously
  SNMPRequestHandler::Response
  process_request_sync(const SNMPRequestHandler::Request &request);
  ...
  struct RequestHandlerStatistics {
    ...
    RequestHandlerStatistics()
        : total_requests(0), successful_requests(0), failed_requests(0),
          v1_requests(0), v2c_requests(0), v3_requests(0),
          total_processing_time(0), average_processing_time(0),
          max_processing_time(0), min_processing_time(0) {}
  };
  ...
  std::chrono::milliseconds get_request_timeout() const {
...
// Utility functions
std::string
thread_pool_statistics_to_string(const ThreadPool::Statistics &stats);
std::string request_handler_statistics_to_string(
    const SNMPThreadPoolManager::RequestHandlerStatistics &stats);

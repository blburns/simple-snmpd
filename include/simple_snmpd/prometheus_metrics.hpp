...
struct PrometheusHistogramBucket {
    ...
    PrometheusHistogramBucket(double bound, uint64_t c) : upper_bound(bound), count(c) {}
};
...
public:
    PrometheusMetric(const std::string& name, const std::string& help, 
                    PrometheusMetricType type, const std::vector<std::string>& label_names = {});
    ...
    const std::vector<std::string>& get_label_names() const { return label_names_; }
    
    // Counter operations
    void increment(double value = 1.0, const std::map<std::string, std::string>& labels = {});
    void set_counter_value(double value, const std::map<std::string, std::string>& labels = {});
    
    // Gauge operations
    void set_gauge_value(double value, const std::map<std::string, std::string>& labels = {});
    void add_gauge_value(double value, const std::map<std::string, std::string>& labels = {});
    void subtract_gauge_value(double value, const std::map<std::string, std::string>& labels = {});
    ...
    // Summary operations
    void observe_summary(double value, const std::map<std::string, std::string>& labels = {});
...
public:
    ...
    void increment_requests_total(const std::string& version, const std::string& pdu_type, 
                                 const std::string& result);
    void increment_requests_duration_seconds(double duration, const std::string& version);
    void increment_requests_size_bytes(size_t size, const std::string& version);
    
    // SNMP error metrics
    void increment_errors_total(const std::string& error_type, const std::string& version);
    ...
    // SNMP MIB metrics
    void increment_mib_queries_total(const std::string& mib_name, const std::string& oid);
    void increment_mib_updates_total(const std::string& mib_name, const std::string& oid);
...
std::string format_prometheus_timestamp(std::chrono::system_clock::time_point timestamp);

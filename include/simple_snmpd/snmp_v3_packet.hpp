...
enum class SNMPv3MessageProcessingModel : uint8_t {
    V3 = 3
};

// SNMP v3 Security Model
enum class SNMPv3SecurityModel : uint8_t {
    USM = 3
};
...
struct SNMPv3MessageFlags {
    ...
    SNMPv3MessageFlags() : reportable(false), privacy(false), 
                          authentication(false), reserved(0) {}
    
    uint8_t to_byte() const {
        return (reportable ? 0x80 : 0) |
               (privacy ? 0x40 : 0) |
               (authentication ? 0x20 : 0) |
               (reserved & 0x1F);
    }
...
public:
    ...
    void set_message_flags(const SNMPv3MessageFlags& flags) { message_flags_ = flags; }
    SNMPv3MessageFlags get_message_flags() const { return message_flags_; }
    
    void set_security_model(SNMPv3SecurityModel model) { security_model_ = model; }
    SNMPv3SecurityModel get_security_model() const { return security_model_; }
    
    // Security parameters
    void set_security_parameters(const SNMPv3SecurityParameters& params) { security_params_ = params; }
    SNMPv3SecurityParameters get_security_parameters() const { return security_params_; }
    
    // Scoped PDU
    void set_scoped_pdu(const SNMPv3ScopedPDU& scoped_pdu) { scoped_pdu_ = scoped_pdu; }
...
private:
    ...
    bool parse_security_parameters(const std::vector<uint8_t>& data, size_t& offset);
...
public:
    ...
    struct Statistics {
        ...
        Statistics() : messages_processed(0), messages_authenticated(0),
                      messages_encrypted(0), messages_decrypted(0),
                      access_checks(0), access_allowed(0), access_denied(0),
                      parse_errors(0), security_errors(0) {}
    };
...
// Utility functions
std::string message_processing_model_to_string(SNMPv3MessageProcessingModel model);
...
SNMPv3MessageProcessingModel string_to_message_processing_model(const std::string& str);
